...
export default function App() {
  ...
  if (currentView === 'admin') {
    return (
      <div className="min-h-screen bg-slate-100 font-sans">
        ...
        {toastMessage && (
          <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-slate-900 text-white px-5 py-3 rounded-2xl shadow-2xl flex items-center gap-2.5 text-xs sm:text-sm font-semibold border border-slate-700 animate-in fade-in slide-in-from-bottom duration-200">
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans bg-[#F4F8FB] text-slate-800 antialiased pb-16 sm:pb-0" id="vinastudent-app">
      ...
      {/* Floating Toast Notification */}
      {toastMessage && (
        <div className="fixed bottom-20 sm:bottom-6 left-1/2 -translate-x-1/2 z-50 bg-slate-900 text-white px-5 py-3 rounded-2xl shadow-2xl flex items-center gap-2.5 text-xs sm:text-sm font-semibold border border-slate-700 animate-in fade-in slide-in-from-bottom duration-200">
          <CheckCircle className="w-5 h-5 text-emerald-400 shrink-0" />
          <span>{toastMessage}</span>
        </div>
      )}
